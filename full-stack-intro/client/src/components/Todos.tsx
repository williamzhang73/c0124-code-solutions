  ...
  useEffect(() => {
    async function work() {
      try {
        ...
        if (!response.ok) throw new Error('Response get method not ok.');
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    async function postData() {
      try {
        ...
        if (!response.ok) throw new Error('Response post method not ok.');
        const data = await response.json();
  ...
  async function toggleCompleted(todo: Todo) {
    async function updateData() {
      try {
        ...
        if (!response.ok) throw new Error('Response put method not ok.');
        const data: Todo = await response.json();
        const newTodos = [...todos];
        for (let i = 0; i < newTodos.length; i++) {
          if (newTodos[i].todoId === data.todoId) {
            newTodos[i] = data;
          }
        }
        setTodos(newTodos);
